#!/usr/bin/env python3
"""duizuo update checker — 7d throttled, non-blocking, provenance-aware.
...
- throttles to 7 days via cache file (XDG cache or skill-root/.cache)
...
SKILL_NAME = "duizuo_reading_companion"
DEFAULT_REPO = "locoda/duizuo-reading-companion-skill"
DEFAULT_BRANCH = "main"
CACHE_TTL_SECONDS = 7 * 24 * 3600  # 1 week as requested
...
def cache_path(root: Path) -> Path:
    ...
    if xdg:
        p = Path(xdg) / "duizuo-reading-companion"
    else:
        p = Path.home() / ".cache" / "duizuo-reading-companion"
    try:
...
def fetch_remote_sha(repo: str, branch: str = DEFAULT_BRANCH) -> tuple[str | None, str, float]:
    ...
    try:
        ...
        req = Request(url, headers={"Accept": "application/vnd.github.v3+json", "User-Agent": f"{SKILL_NAME}-update-checker/1.0"})
        if token:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="duizuo 7d throttled update checker (non-blocking)")
    ...
    if args.json:
        ...
        if update_available:
            ...
            print(f"  gh skill update duizuo-reading-companion  # or: git pull origin {args.branch}")
        else:
