...
- 具体情节、引语、事件顺序及其他书内事实必须说明证据层级。若有电子本，先确定「允许语料」。已有可靠成员映射时，第一个内容承载调用必须显式携带当前阅读单元、锚点与成员范围，只把范围切片写到 `/tmp`，再检索该切片。只有逻辑锚点、尚不知道 EPUB 内部成员时，允许先执行一次**仅元数据边界定位**：工具以当前阅读单元与锚点为输入，可在内部解析 `container.xml`、OPF manifest/spine、NCX 及 OPF 标记的 nav 导航资源，但不得打开 spine 中的 XHTML 正文资源；模型侧只接收解析器签发的本轮临时不透明范围句柄与定位置信度，不接收原始 member ID、文件名、路径、目录标题、其他单元或正文。随后第一个内容承载调用必须携带阅读单元、锚点与该范围句柄，并只在 `/tmp` 生成已读范围切片。定位不唯一就不读取正文。不得先列目录、建全文索引、读取或搜索整本后再过滤。若读者确认没有整本，可在不越过进度边界的前提下查公开简介、书评、访谈等二手来源；答复必须归因，并把来源与核查限制写入 `book.md`。二手来源不能冒充原文，直接引语仍必须可核。
...
| 问题需要文本、逻辑锚点明确，但 EPUB 成员未知 | 先做仅元数据边界定位；模型侧只接收本轮不透明范围句柄与置信度 | 定位不唯一则不读正文，转用用户片段、范围安全的外部来源或核查路径 |
...
- **不把本体可用当作权限**：整本电子书存在不等于允许搜索整本。已有可靠成员映射时，第一个**内容承载**调用必须带有明确的阅读单元、锚点与成员范围，并且只在 `/tmp` 输出范围切片。成员未知时只允许先做一次仅元数据边界定位；定位器除 OPF 标记的 nav 导航资源外，不得打开 spine 中的 XHTML 正文资源，模型侧只接收本轮临时不透明范围句柄与置信度，不接收原始 member ID、文件名、路径、目录标题、其他单元或正文。随后第一个内容承载调用必须携带阅读单元、锚点与该句柄，并只输出 `/tmp` 范围切片。无法在工具参数和轨迹中证明边界就不读取内容。
